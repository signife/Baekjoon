...
int main(void)
{
    ...
    {
        bool escape = false;
        queue<pair<int, int>> fq = {}, sq = {}; // memset때문에 {} 선언?
        cin >> h >> w;                          // 열, 행 입력.( 입력 순서 자체는 행 열임.)
        for (int i = 0; i < w; i++)
        {
            fill(fire_vistied[i], fire_vistied[i] + h, 0);
            fill(sangun_visited[i], sangun_visited[i] + h, 0);
        }
        ...
        while ((!sq.empty()) && (!escape))
        {
            ...
            for (int dir = 0; dir < 4; dir++)
            {
                ...
                if (nx < 0 || ny < 0 || nx >= w || ny >= h)
                {
                    ...
                    cout << sangun_visited[cur.first][cur.second] << '\n';
                    escape = true;
                    break;
                }
                if (board[nx][ny] == -1)
